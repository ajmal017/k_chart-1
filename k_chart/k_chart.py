...
def k_chart(self, 
    ...
    ax_main = fmt.set_ax_format(ax_main, date_tickers, fig) 
    ...
    if main_indicator_cols != None:        
        plotter.plot_main(ax_main, self[main_indicator_cols].values)
    
    # Plot Volume Figure
    if ax_vol != None:
        plotter.plot_vol(ax_vol, self[volume_col].values)
        
    # Plot Sub_Indicator Figure
    if ax_sub != None:
        plotter.plot_sub(ax_sub, self[sub_indicator_cols].values)
